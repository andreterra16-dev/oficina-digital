...
// ── Types ──────────────────────────────────────────────────────────────────
// Shape of one persisted slot entry. Pre-reframe sidecars stored a bare
// data-URL string for a slot (see getSlot's normalization below); newer
// entries store this richer record with pan/zoom state alongside the URL.
...
// The whole sidecar document: an id -> entry map, where an entry may still
// be in the legacy bare-string shape until something normalizes it.
type StoredSlots = Record<string, SlotRecord | string>;

// Live pan/zoom view state held per-element (mirrors the persisted
// {s,x,y} fields of SlotRecord, minus the url).
interface ViewState {
  s: number;
  x: number;
  y: number;
...
// Return shape of _geom(): baseline cover/contain geometry for the current
// image + frame box, or null while it can't be computed yet.
...
// The host bridge injected by the omelette runtime. Absent entirely outside
// that runtime (share links, plain preview) — every call site treats it as
// optional and degrades to read-only.
interface OmeletteBridge {
  writeFile?: (path: string, contents: string) => unknown;
}

// Global augmentation: this file is a plain script (no import/export), so
// this merges directly with lib.dom's ambient Window interface.
interface Window {
  omelette?: OmeletteBridge;
}

// `clonable` is a newer ShadowRoot option that some lib.dom versions don't
// type yet — extend the init type locally instead of reaching for `any`.
interface ShadowRootInitWithClonable extends ShadowRootInit {
  clonable?: boolean;
}
...
(() => {
  ...
  const isUnsplashHost = (u: string): boolean => {
    try {
  ...
  const withReferral = (href: string): string => {
    try {
  ...
  // Narrows a slot-store value to its object shape. Used both to normalize
  // legacy bare-string entries and to satisfy the type checker where the
  // in-memory store is known (by construction) to hold only objects.
  function isSlotRecord(v: SlotRecord | string | undefined): v is SlotRecord {
    ...
    loadP = fetch(STATE_FILE)
      ...
      .then((j: unknown) => {
        ...
        if (j && typeof j === 'object') {
          // The sidecar's shape isn't validated at runtime here (same as
          // the original JS) — trusted the same way the rest of this
          // module trusts window.omelette's writes.
          const parsed = j as StoredSlots;
          const merged: StoredSlots = Object.assign({}, parsed, slots);
  ...
  function getSlot(id: string): SlotRecord | null {
    const v = slots[id];
    if (!v) return null;
    return typeof v === 'string' ? { u: v, s: 1, x: 0, y: 0 } : v;
  }

  function setSlot(id: string, val: SlotRecord | null): void {
    if (!id) return;
  ...
  async function toDataUrl(file: File, targetW: number): Promise<string> {
    ...
    try {
      ...
      // A freshly created canvas's 2D context cannot fail to be created —
      // non-null asserted to match the original code's assumption.
  ...
  // Duck-typed narrowing for the many `e.target && e.target.getAttribute &&
  // e.target.getAttribute(...)` checks in the original — EventTarget has no
  // getAttribute, so a small guard replaces the inline duck-typing without
  // changing what it accepts (anything with a callable getAttribute).
  function hasGetAttribute(t: EventTarget | null): t is Element {
    return !!t && typeof (t as Element).getAttribute === 'function';
  }
  ...
  class ImageSlot extends HTMLElement {
     ...
     *  behavior). */
    static cloneSlot(fromId: unknown, isFree?: (id: string) => boolean): string | null {
    ...
    private _hidShowing = false;
    private _view: ViewState = { s: 1, x: 0, y: 0 };
    private _subFn!: () => void;
    // Session-local copy of the {u,s,x,y} value for id-less slots (they
    // cannot persist to the sidecar, so the drop only lives in memory).
    private _local: SlotRecord | undefined;
    ...
    private _pagehide: (() => void) | null = null;
    private _reposition: (() => void) | null = null;
    private _lastRect = '';
    private _watch: (() => void) | null = null;
    private _watchId = 0;
    private _outside: ((e: Event) => void) | null = null;
    ...
    constructor() {
      ...
      const root: ShadowRoot =
        this.shadowRoot ||
        this.attachShadow({ mode: 'open', clonable: true } as ShadowRootInitWithClonable);
      ...
      this._frame = root.querySelector<HTMLDivElement>('.frame')!;
      this._ring = root.querySelector<HTMLDivElement>('.ring')!;
      this._img = root.querySelector<HTMLImageElement>('.frame img')!;
      this._empty = root.querySelector<HTMLDivElement>('.empty')!;
      this._cap = root.querySelector<HTMLDivElement>('.cap')!;
      this._sub = root.querySelector<HTMLDivElement>('.sub')!;
      ...
      this._ghost = root.querySelector<HTMLImageElement>('.ghost')!;
      this._err = null;
    ...
    private _exitReframe(commit: boolean): void {
      if (!this.hasAttribute('data-reframe')) return;
    ...
    private _signalReframe(active: boolean): void {
      const target: EventTarget = this.isConnected ? this : document;
    ...
    openFilePicker(): void { this._exitReframe(true); this._input.click(); }
    ...
    handleEvent(e: Event): void {
      if (e.type === 'dragenter' || e.type === 'dragover') {
        ...
        e.preventDefault();
        e.stopPropagation();
        // handleEvent is only wired to drag* listeners, so e is always a
        // DragEvent here — EventListenerObject's handleEvent must accept
        // the generic Event type, hence the cast.
    ...
    private async _ingest(file: File): Promise<void> {
      this._setError(null);
      ...
      try {
        ...
        this._swapGen = 0;
        const val: SlotRecord = { u: url, s: 1, x: 0, y: 0 };
        setSlot(this.id || '', val);
    ...
    private _setError(msg: string | null): void {
      ...
      // Shadow root is attached synchronously in the constructor, so it is
      // always present by the time _setError can run.
    ...
    private _releaseMask(settled?: boolean): void {
      if (
        !this._swapGen &&
        !this._loadPending &&
    ...
    // pan toward zero.
    private _geom(): SlotGeometry | null {
    ...
    private _commitView(): void {
      const v: SlotRecord = { s: this._view.s, x: this._view.x, y: this._view.y };
    ...
    private _render(): void {
      ...
      let stored: SlotRecord | null | undefined = this.id ? getSlot(this.id) : this._local;
      ...
      if (showCredit) {
        ...
        const mkLink = (text: string, linkHref: string): HTMLAnchorElement => {
