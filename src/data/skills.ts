 ...
 *
 * Per-branch layout: anchor ± 7 at tier 2, ± 10 at tier 3, anchor − 5 at the
 * single tier-4 node (tapering back in, so the branch reads as fan-out-then-
 * converge rather than ever-widening). Anchors: ia = 18, web = 50 (both
 * matching core's vertical center), dados = 82 — bands end up ≈[8,28],
 * [40,60], [75,89], each comfortably inside the canvas with room to spare
 * top and bottom. Dados has no tier-3 *pair* (docker converges alone from
 * sql+pg, one tier earlier than ia/web's agent/auto and node/ts pairs) —
 * its tier-3 node sits exactly on the anchor, the value a symmetric ±10
 * pair would average out to.
 */
export const SKILLS: NonEmptyArray<Skill> = [
  ...
  { id: 'ux', label: { pt: 'UI de produto', en: 'Product UI' }, kind: { pt: 'peça', en: 'piece' }, b: 'web', x: 80, y: 45, t: 4, lvl: 72, desc: { pt: 'Layout, hierarquia e microinterações: interface que explica sozinha o que o sistema faz.', en: 'Layout, hierarchy, and micro-interactions: an interface that explains by itself what the system does.' } },
  ...
  { id: 'dash', label: { pt: 'Dashboards', en: 'Dashboards' }, kind: { pt: 'peça', en: 'piece' }, b: 'dados', x: 80, y: 77, t: 4, lvl: 76, desc: { pt: 'Painéis que transformam dados brutos em leitura rápida, herança direta dos relatórios técnicos.', en: 'Dashboards transforming raw data into quick reading, a direct heritage of technical reports.' } },
];
